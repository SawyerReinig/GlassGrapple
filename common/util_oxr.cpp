...
oxr_enumerate_instance_ext ()
{
    ...
    xrEnumerateInstanceExtensionProperties(nullptr, 0, &ext_count, nullptr); // First call to get count

//    uint32_t ext_count = 0;
//    OXR_CHECK (xrEnumerateInstanceExtensionProperties (NULL, 0, &ext_count, NULL));

    std::vector<XrExtensionProperties> extProps(ext_count); // ✅ Dynamically allocate array
    for (auto& ext : extProps) {
        ext.type = XR_TYPE_EXTENSION_PROPERTIES;
    }
    xrEnumerateInstanceExtensionProperties(nullptr, ext_count, &ext_count, extProps.data()); // Second call to get extensions
